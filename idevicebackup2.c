...
static plist_t mobilebackup_factory_info_plist_new(const char* udid, idevice_t device, afc_client_t afc)
{
	...
	plist_dict_set_item(ret, "Last Backup Date", plist_new_unix_date(time(NULL)));
	...
	if (cur_dir) {
		...
		while ((ep = readdir(cur_dir))) {
			...
			if (fpath) {
				...
				plist_dict_set_item(fdict, "DLFileModificationDate", plist_new_unix_date(st.st_mtime));
